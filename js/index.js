$(function(){
    ...
    if($('.lead').size()>0){//导航页面
        ...
        var app = new Vue({
            ...
            created:function(){
                ...
                $.ajax({//发起请求
                    ...
                    type: 'get',
                    url:weixin_url+'/sale/template/'+str,
                    contentType:"application/json",
                    success: function(data){
                        that.loading = false;
                        that.showpage = true;
                        if (data.code == 0) {
                            ...
                            $('.good_img img').css('width','100%');
                            $('.good_img img').css('height','auto');
                        }else{
                            alert(data.message);
                        }
                        to_login(data);
                    ...
                    if (that.ope_type == "add_cart") {//如果是加入购物车
                        ...
                        // }else{
                            $.ajax({//发起请求
                                ...
                                data: JSON.stringify({productId:_id,num:_num}),
                                contentType:"application/json",
                                success: function(data){
                                    if (data.code == 0) {
                                        var $toast = $('#toast');
        ...
        var app = new Vue({
            ...
            methods:{
                ...
                Settlement:function(){//结算
                    var _id = $("input[type='checkbox']:checked").parents('.each_row').find(".num_change .ope_num");//id集合
                    var good_info = this.user_info;
                    var _idList = [];
                    var total_price = $('.price_total').html();
                    if (_id.length>0) {
                        ...
                        localStorage.setItem("total_price",total_price);
                        window.location.href='./good-sure.html';
                    }else{
                        alert("请选择商品！");
                    }
                },
                delete_order:function(){//删除按钮
        ...
        var app = new Vue({
            ...
            created:function(){
                ...
                $.ajax({//发起请求获取神庭币信息
                    ...
                    success: function(data){
                        if (data.code == 0) {
                            that.total_coin = data.object.coin;
                            that.need_use = data.object.coin;
                        }else{
                            alert(data.message);
                        }
                        to_login(data);
            ...
            methods:{
                ...
                choose_send:function(e){//快递
                    var that = this;
                    that.transWay = 'trans_way_express';
                    localStorage.setItem("transWay",'trans_way_express');
                },
                choose_self:function(){//自取
                    var that = this;
                ...
                is_coin:function(){//使用神庭币
                    ...
                    if (flag) {//选择使用神庭币
                        ...
                        if(total_price<=total_coin){// 神庭币大于等于商品价格
                            this.total_price = '0.00';
                            this.use_coin = true;//决定下个页面是否起调微信支付
                        }else{//神庭币小于商品价格
                            this.total_price = (total_price - total_coin).toFixed(2);
                            this.use_coin = false;
                        }
                    }else{
                        this.coinPay = '1';
                        this.total_price = this.need_pay;
                        this.total_coin = this.need_use;
                        this.use_coin = false;
                    }
                    localStorage.setItem("use_coin",this.use_coin);
                },
                ...
                coin_pay:function(){//输入密码后点击确认
                    ...
                    if(payPassword.length>0){
                        ...
                        $.ajax({//发起请求
                            ...
                            contentType:"application/json",
                            data: JSON.stringify({"orderNum":orderNum,thirdPay:"ZERO_PAY",payPassword:payPassword,code:code}),
                            async:false,
                            success: function(data){
                                if (data.code == 0) {
    ...
    }else if($('#order_sure').size()>0){//待支付订单的重新确认支付
        var _token = localStorage.getItem('access_token');
        var total_price = localStorage.getItem('total_price');
        var order_info =JSON.parse(localStorage.getItem('order_info'));
        var good_info =order_info.saleOrderProductList;
        var app = new Vue({
            el: '#order_sure',
            data: {
                new_info:good_info,
                coinpay_num:order_info.orderNum,//余额支付的订单号
            ...
            methods:{
                pay_sure:function(){//确认支付按钮
                    var that = this;
                    var _token = localStorage.getItem('access_token');
                    var payPrice = that.order_info.payPrice;
        ...
        $('#order_sure #quit_account2 button.time_cancel').live('click',function(){//取消按钮
            $('#quit_account2').css("display","none");
            window.location.href="../html/pay-list.html";
        });
        $('#order_sure #quit_account3 button.time_cancel').live('click',function(){//取消按钮
            var $androidActionSheet = $('#quit_account3');
            var $androidActionSheet2 = $('#quit_account2');
            var $androidMask = $androidActionSheet.find('.weui-mask3');
            var $androidMask2 = $androidActionSheet2.find('.weui-mask2');
        ...
        // 头部菜单切换
        $('#order_sure .bar_option').on('click','.flex-1',function(){
            clearTimeout(set_time);
            $('.get_code .remain_time').html('60');//初始化
            if($(this).hasClass('active')){
                $(this).siblings().removeClass('active');
        ...
        var app = new Vue({
            ...
            methods:{
                ...
                cancel_delete:function(){//取消删除
                    var $androidActionSheet = $('#quit_account');
                    $androidActionSheet.css('display','none');
                },
                sure_delete:function(e){//确认删除
                    var that = this;
                    var delete_id ="" + that.delete_id;
                    $.ajax({//发起请求
                        headers: {
                            'Authorization': 'bearer '+_token
                        },
                        type: 'post',
                        url:medicine_url+'/v1.0.0/personalCenter/deleteLogisticsAddress',
                        contentType:"application/json",
        ...
        $('.add_addr').live('click',function(){
            window.location.href="../html/add-address.html?status=add";
        });
    }else if($('#add_address').size()>0){//编辑地址页面
        var _token = localStorage.getItem('access_token');
        var _url = window.location.href;
        var _index = _url.lastIndexOf("\=");  
        var str  = decodeURI(_url.substring(_index + 1, _url.length));
                    ...
                    success: function(data){
                        if (data.code == 0) {
                            ...
                            if (str=='edit'){//编辑
                                ...
                                // // 再修改页面内容
                                var _city = addr_info.locationSeqcn.split('-');
                                var _city_id = addr_info.locationSeqno.split('.');
                                that.province.id = _city_id[0];
                                that.city.id = _city_id[1];
                                that.area.id = _city_id[2];
                                that.province.name = _city[0];
    ...
    }else if($('#sale_pay').size()>0){//馆藏商品支付页面
        var _url = window.location.href;
        var code = _url.match(/=(\S*)&/)[1];
        var _token = localStorage.getItem('access_token');
        ...
        function get_info(order_status,page,that){
            ...
            if (page < total_page) {//未加载到最后一页
                $.ajax({
                    ...
                    type: "get",
                    url: weixin_url + "/sale/order/"+order_status+"?page="+page,
                    contentType:"application/json",
                    dataType: "json",
                    success: function(data){
                        if(data.code == 0){//请求数据正常
        ...
        var app = new Vue({
            ...
            methods:{
                handleScroll:function(event){//处理滚动事件
                    ...
                    var scrollTop = window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop
                    // div高度
                    var offsetHeight = document.querySelector('#pay_list').offsetHeight;
                    // 页面高度
                    var clientHeight =  window.screen.height ; 
                    if (clientHeight+scrollTop+10>offsetHeight){
                        var page = that.page+1;
                ...
                change_status: function (event) {
                    ...
                    if (is_active !==-1) {//被选中
                        ...
                        that.all_good=[];
                        if(class_name2 == 'success'){
                            var order_status = 'success';
                            that.order_status = 'success';//改变页面内容
                        }else if(class_name2 == 'wait-for-pay'){
                ...
                to_sale:function(e,_idList,total_price){//跳转到结算页面
                    if (e=='wait-for-pay') {
                        localStorage.setItem("order_info",JSON.stringify(_idList));
        ...
        $('#pay_list #quit_account2 button.time_cancel').live('click',function(){//取消按钮
            $('#quit_account2').css("display","none");
        });
        $('#pay_list #quit_account3 button.time_cancel').live('click',function(){//取消按钮
            var $androidActionSheet = $('#quit_account3');
            var $androidActionSheet2 = $('#quit_account2');
            var $androidMask = $androidActionSheet.find('.weui-mask3');
            var $androidMask2 = $androidActionSheet2.find('.weui-mask2');
        ...
        // 头部菜单切换
        $('#pay_list .bar_option').on('click','.flex-1',function(){
            clearTimeout(set_time);
            $('.get_code .remain_time').html('60');//初始化
            if($(this).hasClass('active')){
                $(this).siblings().removeClass('active');
